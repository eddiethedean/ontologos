...
- [x] **B3 — Internal test ports** — **Complete (2026-06-29, portable scope):** 24/24 `NormalizationTest` + 8/8 `ClausificationTest` → `migrated`; 33 clausify OFN catalog; **7/7** RDF/XML hyper goldens. **Hyper clausify:** `hyper_object`, `hyper_nominals`, `hyper_cardinality`, `hyper_abox`. **Inventory (defer full port):** 23 `tableau.*` ([tableau_internals.rs](crates/ontologos-alc/tests/tableau_internals.rs)), 3 `graph.*` ([graph_internals.rs](crates/ontologos-alc/tests/graph_internals.rs)) — blocked on DescriptionGraph/extension-manager hooks. Mapping: [internal_ports.toml](tests/hermit/internal_ports.toml)
- [ ] **B4 — Literal catalog burn-down** — Harness metrics on `parity_status` (`literal_catalog_pct`, `taxonomy_strict_pct`, `perf_gate_pct`); **122** `#[ignore]` tests (`cargo test -p ontologos-conformance -- --ignored` nightly); promote via `hermit-burndown.sh promote`
- [x] **C — Strict taxonomy (harness)** — `reduce_transitive_redundancy()` in classify (done); nightly **`compare-tier-c-strict-family.sh`** with `ONTOLOGOS_STRICT_TAXONOMY=1` (informational); **`taxonomy_strict_pct`** on `parity_status`. **Remaining:** engine burn-down of HermiT extra edges on family/pizza/go-subset
- [x] **D — Perf + API (partial)** — **`compare-tier-d-perf-gate.sh`** (Family DL **< 1.0 s** PR); Criterion `classify` + `saturation` benches; facade **`is_entailed`**, **`query_engine`**; CLI **`consistent`** / **`entail`**. **Remaining:** Pizza **< 30 s**, SWRL full catalog, Python bindings
