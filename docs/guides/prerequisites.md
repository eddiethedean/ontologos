...
OntoLogos is a **Rust workspace** with optional **Python bindings** and **source-build native bindings** (v1.1.3 on `main`). Pick the row that matches your task. Install channels: [Install and channels](install-channels.md).
...
Tagged install: [CLI installation](../getting-started/cli-install.md) (`--tag v1.1.3`).
...
| npm/Maven/NuGet packages | Bindings are source-build until v1.1.3 publishes |
