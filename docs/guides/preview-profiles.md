...
!!! warning "Preview only (`alc`, `dl-preview`)"
    Preview engines may return `PreviewLimit` or `ResourceLimit`. For production OWL DL, use `--profile dl` (stable on v1.1.3). See [Profile stability matrix](profile-stability.md).
