...
|------------|---------------|----------|
| Embed in **Rust** | `ontologos-* = "1.1.0"` on [crates.io](https://crates.io/crates/ontologos-core) | **Published** |
...
**v1.1.0** adds shared FFI and native bindings. Build bindings from a clone at tag **`v1.1.0`** or from `main`. See [Release status](../project/release-status.md).
...
On **v1.1.0** (PyPI / crates.io): `rdfs`, `rl`, `el`, `auto`, `dl`, `swrl` are production-supported. Preview: `alc`, `dl-preview`.
