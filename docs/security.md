...
Both validate the path, enforce [`ParseLimits`](https://docs.rs/ontologos-parser/0.6.1/ontologos_parser/struct.ParseLimits.html), then parse via horned-owl. Sandboxed loads open the file once with `O_NOFOLLOW` (Unix) and sniff plus parse from the same file descriptor so a symlink swap between validation and read cannot escape the base directory.
