...
Thank you for your interest in contributing. OntoLogos is in active development: **v1.1.1** is the current release on crates.io/PyPI and the **`main`** workspace. See [Release status](docs/project/release-status.md). High-impact contributions include conformance, bindings, documentation, and incremental reasoning polish.
...
4. **Version pins in docs:** Published install blocks must use **1.1.1** (`docs/scripts/check-doc-versions.sh` enforces this). Migration guides may reference older versions for upgrade paths.
...
1. Confirm workspace [Cargo.toml](Cargo.toml) `version = "1.1.1"`.
2. Ensure [CHANGELOG.md](CHANGELOG.md) has a dated `[1.1.1]` section and empty `[Unreleased]`.
3. Run `./docs/scripts/check-doc-versions.sh` and `./docs/scripts/check-doc-snippets.sh`.
4. Commit release prep on `main`.
5. Create an annotated tag: `git tag -a v1.1.1 -m "OntoLogos v1.1.1"`
6. Push commit and tag: `git push origin main && git push origin v1.1.1`
7. The [release workflow](.github/workflows/release.yml) runs when the tag is pushed (requires GitHub secrets below).
8. Create a GitHub Release from [`.github/release/v1.1.1.md`](.github/release/v1.1.1.md).
