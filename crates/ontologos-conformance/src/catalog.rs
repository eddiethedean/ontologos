...
pub fn scan_promotable_axiom_cases() -> Vec<String> {
    ...
    for case in read_catalog_file() {
        if case.status != "planned" || !case_has_axiom_assertions(&case) {
            continue;
        }
        if check_axiom_case(&case).is_ok() {
            passing.push(case.id.clone());
        }
    }
    passing.sort();
    passing
...
pub fn write_promoted_axiom_ids(ids: &[String]) -> std::io::Result<()> {
    ...
    let mut lines = vec![
        ...
        "# Re-run: cargo run -p ontologos-conformance --bin promote_catalog".to_string(),
    ];
